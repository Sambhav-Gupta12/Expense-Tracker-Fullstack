...
function TransactionCard({ showAll, transactions }) {
    ...
    const handleTransactionDelete = async () => {
        try {
            ...
            if (selectedTransaction.transType === "expense") {
                ...
                setExpenses(prev =>
                    prev.filter(exp => exp._id !== selectedTransaction._id)
                );

                toast.success("Expense deleted successfully!");
            }
            else {
                await axios.delete(`${API}/incomes/delete-income/${selectedTransaction._id}`,
                    {
                        withCredentials: true,
                    }
                )
                ...
                setIncomes(prev =>
                    prev.filter(inc => inc._id !== selectedTransaction._id)
                );
    ...
    return (
        <>
            {
                hasTransactions ? (displayedTransactions.map((transaction) => (
...

            {
                showConfirmModal && (
                    <ConfirmDelete
                        ...
                        loading={loading}
                        upperLine="Are you sure you want to delete this transaction?"
                        lowerLine="Your data will be lost !"
                    />
                )
            }
        </>
